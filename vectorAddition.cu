...
// ---------- Vector Addition ----------
_global_ void vectorAddGPU(float *A, float *B, float *C, int n) {
...
_global_ void matMulGPU(float *A, float *B, float *C, int width) {
...
int main() {
    ...
    bool vectorMatch = compareArrays(h_C_cpu, h_C_gpu, N);
    cout << "Vector addition match: " << (vectorMatch ? "Yes ✅" : "No ❌") << endl;
    ...
    bool matrixMatch = compareArrays(C_cpu, C_gpu, size);
    cout << "Matrix multiplication match: " << (matrixMatch ? "Yes ✅" : "No ❌") << endl;
